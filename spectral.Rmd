...
samples_df <-
    cbind(
      as.data.frame(as.matrix(expand.grid(x_))),
      data.frame(
        Truncated = as.vector(S2sample(S_truncated, n, h, seed = 123L)),
        Folded = as.vector(S2sample(S_folded, n, h, seed = 123L)),
        "Folded_cell" = as.vector(S2sample(S_folded_cell, n, h, seed = 123L))
      )
    ) %>%
      pivot_longer(
        cols = c("Truncated", "Folded", "Folded_cell"),
...
Plot only half, since that's likely the correct interpretation.
Need to do some math checking, and to see if this can be improved...
```{r}
ggplot(samples_df, aes(x1, Value)) +
  geom_line(aes(col = Type)) +
  coord_cartesian(xlim = c(0, L/2)) +
  xlab("x") + ylab("Sample")
```
(for long correlation range, the three version are superimposed, since they used the same random seed, and the spectra are almost identical even after folding/truncation)
